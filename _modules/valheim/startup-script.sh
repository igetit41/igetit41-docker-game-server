#!/bin/bash
# Legacy metadata-driven startup script for Valheim. TODO: replace with Valheim-specific script.
...
if [ -f "$STANDARD_REPO/_modules/minecraft/compose.yaml" ] || [ -f "$STANDARD_REPO/_modules/valheim/compose.yaml" ] || [ -f "$STANDARD_REPO/_modules/7d2d/compose.yaml" ]; then
  REPO_ROOT=$STANDARD_REPO
elif [ -f "$FLAT_REPO/_modules/minecraft/compose.yaml" ] || [ -f "$FLAT_REPO/_modules/valheim/compose.yaml" ] || [ -f "$FLAT_REPO/_modules/7d2d/compose.yaml" ]; then
  REPO_ROOT=$FLAT_REPO
else
  REPO_ROOT=$STANDARD_REPO
fi
...
if [[ "$RCON_COMPATIBLE" == "true" ]]; then
    ...
    while [[ "$RCON_CHECK" == "" ]]; do
        ...
        if [[ "$SERVER_CHECK1" == *game-server* ]] && { [[ "$SERVER_CHECK2" == /home* ]] || [[ "$SERVER_CHECK2" == /data ]]; }; then
    ...
    LOOP_VAR=0
    while [[ "$SERVER_CHECK1" != *game-server* ]] && { [[ "$SERVER_CHECK2" != /home* ]] && [[ "$SERVER_CHECK2" != /data ]]; }; do
        LOOP_VAR="$(($LOOP_VAR + 1))"
        echo "-----startup-script-output-LOOP_VAR-$LOOP_VAR"
        echo "-----startup-script-output-waiting-for-server"
        SERVER_CHECK1=$(echo "$(sudo docker ps)" | grep -E game-server)
