...
# Project Zomboid — idle shutdown via RCON players on port 27015.
...
RCON_PORT=27015
RCON_OTHER_ARGS=""
RCON_PW_FILE=channel27.ini
RCON_PW_FILE_PATH=./Zomboid/Server
RCON_PW_VAR=RCONPassword
RCON_PLAYER_CHECK=players
RCON_PLAYER_CHECK_GREP="grep -Eo '[0-9]+'"
RCON_LIVE_TEST=help
RCON_LIVE_TEST_GREP="grep -Eo createhorde"
RCON_COMMANDS="setaccesslevel D3F1L3 admin"
RCON_RELOAD="reloadlua channel27_SandboxVars.lua"
...
RCON_PW_VAR_LINE="RCONPassword=${RCON_PW}"

EXEC_COMMANDS="sed -i '/CharacterFreePoints/s/.*/    CharacterFreePoints = 12,/' ./Zomboid/Server/channel27_SandboxVars.lua"
EXEC_COMMANDS="${EXEC_COMMANDS};sed -i '/StarterKit/s/.*/    StarterKit = true,\\n    MaximumDismantlingXpLevel = 10,\\n    XpMultiplier = 4,/' ./Zomboid/Server/channel27_SandboxVars.lua"
EXEC_COMMANDS="${EXEC_COMMANDS};sed -i '/MinutesPerPage/s/.*/    MinutesPerPage = 0.1,/' ./Zomboid/Server/channel27_SandboxVars.lua"
EXEC_COMMANDS="${EXEC_COMMANDS};sed -i '/WaterShutModifier/s/.*/    WaterShutModifier = 120,/' ./Zomboid/Server/channel27_SandboxVars.lua"
EXEC_COMMANDS="${EXEC_COMMANDS};sed -i '/ElecShutModifier/s/.*/    ElecShutModifier = 60,/' ./Zomboid/Server/channel27_SandboxVars.lua"
EXEC_COMMANDS="${EXEC_COMMANDS};sed -i '/Transmission/s/.*/    Transmission = 3,/' ./Zomboid/Server/channel27_SandboxVars.lua"
EXEC_COMMANDS="${EXEC_COMMANDS};sed -i '/^SleepAllowed/s/.*/SleepAllowed=true/' ./Zomboid/Server/channel27.ini"

STANDARD_REPO=/home/game-server/igetit41-docker-game-server
FLAT_REPO=/home/game-server
if [ -f "$STANDARD_REPO/_modules/zomboid/compose.yaml" ]; then
  REPO_ROOT=$STANDARD_REPO
elif [ -f "$FLAT_REPO/_modules/zomboid/compose.yaml" ]; then
  REPO_ROOT=$FLAT_REPO
else
  REPO_ROOT=$STANDARD_REPO
fi
