...
while [[ $RESTART_COUNT -gt "0" ]]; do
    ...
    docker_command="sudo docker exec -i game-server ./rcon-0.10.3-amd64_linux/rcon -a 127.0.0.1:27015 -p $RCON_PW $RCON_LIVE_TEST"
    echo "-----startup-script-output-docker_command: $docker_command"
    GAMESERVER_RUNNING=$($docker_command)
...
while true; do
    ...
    docker_command="sudo docker exec -i game-server ./rcon-0.10.3-amd64_linux/rcon -a 127.0.0.1:27015 -p $RCON_PW $RCON_PLAYER_CHECK"
    echo "-----startup-script-output-docker_command: $docker_command"
    PLAYERS=$($docker_command)
