...
while [[ "$RCON_FILE_CHECK" == "" ]] || [[ "$PASSWORD_CHECK" != *$RCON_PW* ]]; do
    ...
    if [[ "$RCON_FILE_CHECK" == *$RCON_PW_FILE* ]]; then
        ...
        #REMOVE_EMPTY_PASSWORD=$(sudo docker exec -i game-server bash -c "sed -i '/$RCON_PW_VAR/d' '$RCON_PW_FILE_PATH/$RCON_PW_FILE'")
        REMOVE_EMPTY_PASSWORD=$(sudo docker exec -i game-server bash -c "sed -i 's/$RCON_PW_VAR_LINE1$RCON_PW_VAR_LINE2/$RCON_PW_VAR_LINE1$RCON_PW$RCON_PW_VAR_LINE2/g' '$RCON_PW_FILE_PATH/$RCON_PW_FILE'")
