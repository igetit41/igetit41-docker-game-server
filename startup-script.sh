...
while true; do
    ...
    PLAYERS=$(echo "$(sudo docker exec -i game-server ./rcon-0.10.3-amd64_linux/rcon -a 127.0.0.1:$RCON_PORT -p $RCON_PW $RCON_OTHER_ARGS \"$RCON_PLAYER_CHECK\")" | $RCON_PLAYER_CHECK_GREP | tr -cd '[:digit:].')
